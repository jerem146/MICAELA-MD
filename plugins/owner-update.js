...
let handler = async (m, { conn }) => {
  ...
  await conn.sendMessage(m.chat, { text: '🔄 Actualizando el bot...' }, { quoted: m })

  exec('git pull', { cwd: botPath }, (err, stdout, stderr) => {
    if (err) {
      conn.sendMessage(
        m.chat,
        { text: `❌ Error: No se pudo actualizar el bot.\nRazón: ${err.message}` },
        { quoted: m }
      )
      return
    }
    ...
    if (stdout.includes('Already up to date.')) {
      conn.sendMessage(m.chat, { text: '✅ El bot ya está actualizado.' }, { quoted: m })
    } else {
      conn.sendMessage(
        m.chat,
